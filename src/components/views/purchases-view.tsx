    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-2 mt-2">
          <Popover open={prodOpen} onOpenChange={setProdOpen}><PopoverTrigger asChild><Button variant="outline" size="sm" className="gap-2 h-9"><Search className="w-4 h-4 text-muted-foreground" /> Add product…</Button></PopoverTrigger><PopoverContent className="p-0 w-[380px]" align="start"><Command shouldFilter={false}><CommandInput placeholder="Search…" value={prodQ} onValueChange={setProdQ} /><CommandList><CommandEmpty>No product</CommandEmpty><CommandGroup>{products.filter((p) => !prodQ || p.name.toLowerCase().includes(prodQ.toLowerCase())).slice(0, 8).map((p) => <CommandItem key={p.id} onSelect={() => addProduct(p)}><Package className="w-4 h-4" /><div className="flex-1"><div className="text-[13px] font-medium">{p.name}</div><div className="text-[11px] text-muted-foreground">Stock: {p.stock} · {formatINR(p.purchasePrice)}</div></div></CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover>
        </div>
        {lines.length > 0 && (
