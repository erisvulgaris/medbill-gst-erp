...
export const GET = apiHandler(async (req: NextRequest) => {
  ...
  if (report === "profit_loss") {
    const [invoices, purchases, expenses] = await Promise.all([
      db.invoice.findMany({ where: { businessId: ctx.businessId, deletedAt: null, invoiceDate: { gte: fromD, lte: toD } }, select: { taxableValue: true, grandTotal: true, items: { select: { taxable: true, total: true } } } }),
      db.purchase.findMany({ where: { businessId: ctx.businessId, deletedAt: null, invoiceDate: { gte: fromD, lte: toD } }, select: { taxableValue: true, grandTotal: true } }),
      db.expense.findMany({ where: { businessId: ctx.businessId, date: { gte: fromD, lte: toD } }, select: { amount: true, category: true } }),
    ]);
  ...
  if (report === "party_summary") {
    const parties = await db.party.findMany({ where: { businessId: ctx.businessId, deletedAt: null }, orderBy: { name: "asc" } });
    const partyIds = parties.map((p) => p.id);
    const [sales, purchases, payments] = await Promise.all([
      db.invoice.findMany({ where: { businessId: ctx.businessId, partyId: { in: partyIds }, invoiceDate: { gte: fromD, lte: toD } }, select: { partyId: true, grandTotal: true, balance: true } }),
      db.purchase.findMany({ where: { businessId: ctx.businessId, partyId: { in: partyIds }, invoiceDate: { gte: fromD, lte: toD } }, select: { partyId: true, grandTotal: true } }),
      db.payment.findMany({ where: { businessId: ctx.businessId, partyId: { in: partyIds }, date: { gte: fromD, lte: toD } }, select: { partyId: true, amount: true, type: true } }),
    ]);
    ...
    return apiSuccess({ title: "Party Report", period: { from: fromD, to: toD }, rows });
  }

  if (report === "inventory_valuation") {
    const products = await db.product.findMany({ where: { businessId: ctx.businessId, deletedAt: null }, include: { unit: true } });
    ...
    return apiSuccess({ title: "Inventory Valuation", rows, totals: { totalCost, totalSale, potentialProfit: totalSale - totalCost } });
  }

  if (report === "day_book") {
    const [invoices, purchases, expenses, payments] = await Promise.all([
      db.invoice.findMany({ where: { businessId: ctx.businessId, invoiceDate: { gte: fromD, lte: toD } }, select: { invoiceDate: true, number: true, partyName: true, grandTotal: true } }),
      db.purchase.findMany({ where: { businessId: ctx.businessId, invoiceDate: { gte: fromD, lte: toD } }, select: { invoiceDate: true, number: true, partyName: true, grandTotal: true } }),
      db.expense.findMany({ where: { businessId: ctx.businessId, date: { gte: fromD, lte: toD } }, select: { date: true, category: true, amount: true, note: true } }),
      db.payment.findMany({ where: { businessId: ctx.businessId, date: { gte: fromD, lte: toD } }, select: { date: true, type: true, mode: true, amount: true, reference: true } }),
    ]);
    return apiSuccess({
      title: "Day Book",
      period: { from: fromD, to: toD },
